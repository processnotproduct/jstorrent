function JSTorrentClient() {
    this.filesystem = new FileSystem();
    this.filesystem.request_fs();

    //this.torrents = {};
    this.torrents = new TorrentCollection();
    this.torrents.fetch();
/*
*/
...
JSTorrentClient.prototype = {
    ...
    add_torrent: function(args) {
        ...
        if (args && args.metadata) {
            var torrent = new Torrent( { metadata: args.metadata } );
        } else if (args && args.infohash) {
            assert( args.infohash.length == 40 );
            var torrent = new Torrent( { infohash: args.infohash } );
        } else if (args && args.magnet) {
            var torrent = new Torrent( { magnet: args.magnet } );
        }
